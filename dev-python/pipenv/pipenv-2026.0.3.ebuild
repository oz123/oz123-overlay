...
src_prepare() {
	...
	local pkgName
	local packages=( colorama click click_didyoumean dotenv
					 pexpect pipdeptree plette pyparsing pythonfinder
					 shellingham tomli tomlkit importlib_metadata packaging )
	...
	for dir in pipenv/vendor examples docs; do rm -Rfv "$dir" || die "Failed to remove ${dir}"; done
